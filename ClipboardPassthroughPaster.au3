...
Local $hForm = GUICreate(StringReplace(@ScriptName, '.au3', ''), 400, 400, 10, 10, BitOR($GUI_SS_DEFAULT_GUI, $WS_MAXIMIZEBOX, $WS_SIZEBOX), $WS_EX_TOPMOST)
...
Func WM_CLIPBOARDUPDATE($hWnd, $Msg, $wParam, $lParam)
	...
	Send(_ClipBoard_GetData($CF_TEXT))
	Send("{Enter}")
    _ClipBoard_Close()
    Return 0
